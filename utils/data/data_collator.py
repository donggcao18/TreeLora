...
# HF instruction pool prompts are generated before reaching the collator.
LEGACY_TASK_PROMPT = {
...
class DataCollator:
    ...
    def __call__(self, batch, return_tensors=None):
        ...
        return self.decoder_call(batch, self.return_tensors)
    ...
    def decoder_call(self, batch, return_tensors):
            ...
            if not self.inference:
                ...
                if self.demonstrations is not None:
                    if _is_hf_task(self.task):
                        ...
                        task_prompt = LEGACY_TASK_PROMPT.get(self.task, "")
                ...
                formatted_prompt = f"input: {instruction}\noutput: "
                tokenized_sources.append(
                    self.tokenize(formatted_prompt, limit_len, add_bos_token=False, add_eos_token=False)
                )

            actual_max_len = max(actual_max_len, len(tokenized_sources[-1]["input_ids"]))
        ...
        for idx in range(len(tokenized_sources)):
            ...
            if not self.inference:
                tokenized_sources[idx]["input_ids"] = tokenized_sources[idx]["input_ids"] + [pad_id] * pad_len
                tokenized_sources[idx]["attention_mask"] = tokenized_sources[idx]["attention_mask"] + [0] * pad_len
                tokenized_sources[idx]["labels"] = tokenized_sources[idx]["labels"] + [-100] * pad_len
                assert (
                ...
                tokenized_sources[idx]["input_ids"] = [pad_id] * pad_len + tokenized_sources[idx]["input_ids"]
